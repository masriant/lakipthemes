...
<div id="post-<?php the_ID(); ?>" class="elegantwp-fp09-post">
    ...
    <div class="elegantwp-fp09-post-thumbnail">
        <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp' ), the_title_attribute( 'echo=0' ) ) ); ?>" class="elegantwp-fp09-post-thumbnail-link"><?php the_post_thumbnail('elegantwp-featured-image', array('class' => 'elegantwp-fp09-post-thumbnail-img')); ?></a>
    </div>
    <?php } ?>
    <?php } ?>
    ...
    the_content( sprintf(
        wp_kses(
            ...
            __( 'Continue reading<span class="screen-reader-text"> "%s"</span> <span class="meta-nav">&rarr;</span>', 'elegantwp' ),
            array(
                'span' => array(
                    'class' => array(),
                ),
    ...
    wp_link_pages( array(
     'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'elegantwp' ) . '</span>',
     'after'       => '</div>',
     'link_before' => '<span>',
     'link_after'  => '</span>',
     'separator'   => '',
    ...
    <?php if ( !(elegantwp_get_option('hide_post_tags_home')) ) { ?>
