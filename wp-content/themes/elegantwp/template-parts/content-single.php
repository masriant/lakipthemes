                ...
                if ( !(elegantwp_get_option('hide_thumbnail')) ) {
                    if ( !(elegantwp_get_option('hide_thumbnail_single')) ) {
                        if ( elegantwp_get_option('thumbnail_link') == 'no' ) {
                            ...
                            <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-featured-image', array('class' => 'elegantwp-post-thumbnail-single')); ?></a>
                <?php   }
                    }
                }
            }
