...
function elegantwp_setup() {
     ...
     * to change 'elegantwp' to the name of your theme in all the template files.
     */
    load_theme_textdomain( 'elegantwp', get_template_directory() . '/languages' );
    ...
    'primary' => esc_html__('Primary Menu', 'elegantwp'),
    'secondary' => esc_html__('Secondary Menu', 'elegantwp')
    ) );
...
function elegantwp_content_width() {
    $elegantwp_content_width = 680;

    if ( is_page_template( 'template-full-width-page.php' ) ) {
        $elegantwp_content_width = 1230;
    }
    ...
    $GLOBALS['content_width'] = apply_filters( 'elegantwp_content_width', $elegantwp_content_width ); /* phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedVariableFound */
}
